...
[...defaultWorkItems, ...bellaItems].forEach((item) => {
  const img = new Image();
  img.src = item.image;
});

let workTransitionTimer;
...
function showWork(index) {
  ...
  clearTimeout(workTransitionTimer);
  ...
  workTransitionTimer = setTimeout(() => {
...
document.addEventListener('keydown', (event) => {
  ...
  if (typed === secret) {
    typed = '';
    activateBellaServices();
  }
}, true);

function activateBellaServices() {
  document.body.classList.add('bella-services');

  const toast = document.getElementById('toast');
  if (toast) {
    toast.innerHTML = '🐾 <strong>Bella Mode</strong>';
